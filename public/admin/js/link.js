layui.define(['laypage', 'layer', 'form', 'pagesize'], function(exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        function htmlTo(data) {
            ...
            html += '<thead><tr><th>发表时间</th><th>名称</th><th>状态</th><th  colspan="2">操作 <a id="addArticle" class="layui-btn layui-btn-normal">新增友情链接</a></th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                ...
                html += "<tr>";
                html += "<td>" + new Date(parseInt(item.LinkTimer)).format("yyyy-MM-dd HH:mm:ss") + "</td>";
                html += "<td>" + item.LinkName + "</td>";
                html += "<td>" + item.LinkState + "</td>";
                html += '<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.link.editData(\'' + item._id + '\')"><i class="layui-icon">&#xe642;</i></button></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-danger" onclick="layui.link.deleteData(\'' + item._id + '\')"><i class="layui-icon">&#xe640;</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
            ...
            $('#linkConsole,#link').attr('style', 'display:block'); //显示FiledBox
